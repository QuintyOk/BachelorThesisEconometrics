...
target_var <- "medv"
selected_vars <- c("crim", "dis", "indus", "lstat", "nox")
boston_selected_vars <- boston %>% select(c(target_var, selected_vars))
...
# Function to calculate usual variable importance
...
folds <- createFolds(boston$medv, k = 5, list = TRUE)
for (fold in 1:5) {
  cat("++++ Processing fold ", fold, "+++++ \n")
  train_idx <- unlist(folds[-fold])
  test_idx <- unlist(folds[fold])
  
  boston_train <- boston_selected_vars[train_idx, ]
  boston_test <- boston_selected_vars[test_idx, ]
  ...
  for (predictor in selected_vars) {
    ...
    varimp_scores <- compute_conditional_importance(predictor, boston_train, selected_vars)
  ...
  for (q in quantiles) {
    ...
    qrf_model <- quantregForest(x = select(boston_train, -medv), y = boston_train$medv, ntree = 1000, mtry = 2, nodesize = 10)
    cut_points <- extract_cut_points(qrf_model)
    
    for (var in selected_vars) {
      print(paste("For :", var, "at quantile", q))
      conditional_importance <- conditional_permutation_importance(qrf_model, boston_test, var_interest = var, n_permutations = 100, conditioning_vars = conditioning_vars, cut_points = cut_points, threshold = 0.05, alpha = q)
      usual_importance <- usual_variable_importance(qrf_model, boston_test, var_interest = var, n_permutations = 100, alpha = q)
...
importance_results_aggregated_boston_sample <- importance_results_cv %>%
...
print(importance_results_aggregated_boston_sample)
write.csv(importance_results_aggregated_boston_sample, file = "boston_importance_final_sample.csv", row.names = FALSE)
